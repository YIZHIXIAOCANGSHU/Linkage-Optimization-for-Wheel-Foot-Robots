...
%% ========== 遍历k值计算 ==========
s_values = 0.01:0.01:1;  % k值范围从0.01到1，步进0.01 (避免k=0时的除零错误)
F1_values = zeros(size(s_values));
F2_values = zeros(size(s_values));
l4_val = s_values * l2_val;  % 连杆长度(m)
...
for i = 1:length(s_values)
    ...
    if Cy1_num <= 0 && Cy2_num <= 0
        warning('k=%.2f时，两个解的y值都小于等于0，不符合物理意义！');
        % 将无效值设为NaN
        F1_values(i) = NaN;
        F2_values(i) = NaN;
        continue;
    ...
    % 直接代入当前循环的参数值
    params = struct('l1', l1_val, 'l2', l2_val, 'k', k_val, 'theta1', theta1_val, 'theta2', theta2_val, 'l4', l4_val(i));
    ...
    if abs(denominator_dGH_val) < eps || isinf(denominator_dGH_val) || isnan(denominator_dGH_val)
        ...
        if abs(d_GH_val) < eps || isinf(d_GH_val) || isnan(d_GH_val)
            ...
            F1_values(i) = double(subs(F1, params));
        end
    end
    ...
    if abs(DC_magnitude_val) < eps || isinf(DC_magnitude_val) || isnan(DC_magnitude_val)
        ...
        if abs(d_CE_val) < eps || isinf(d_CE_val) || isnan(d_CE_val)
            ...
            if abs(CE_magnitude_val) < eps || isinf(CE_magnitude_val) || isnan(CE_magnitude_val)
                ...
                F2_values(i) = double(subs(F2, params));
            end
        end
    end
...
%% ========== 可视化 ==========
figure;
plot(s_values, F1_values, 'b-', 'LineWidth', 1.5);
hold on;
plot(s_values, F2_values, 'r--', 'LineWidth', 1.5);
xlabel('s值');
ylabel('负载力 (N)');
title('F1和F2随s值的变化曲线');
legend('F1', 'F2');
