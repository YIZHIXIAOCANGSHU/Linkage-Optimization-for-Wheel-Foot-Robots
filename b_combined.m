...
%% ========== 第一部分：固定k=0.5，遍历s值计算 ==========
k_val = 0.5;  % 固定k值用于s值变化分析

s_values = 0.01:0.01:1;  % s值范围从0.01到1，步进0.01 (避免s=0时的除零错误)
F1_s_values = zeros(size(s_values));
F2_s_values = zeros(size(s_values));
l4_s_val = s_values * l2_val;  % 连杆长度(m)
...
for i = 1:length(s_values)
    ...
    if Cy1_num <= 0 && Cy2_num <= 0
        warning('s=%.2f时，两个解的y值都小于等于0，不符合物理意义！', s_values(i));
        % 将无效值设为NaN
        F1_s_values(i) = NaN;
        F2_s_values(i) = NaN;
        continue;
    ...
    if abs(DC_magnitude_val) < eps || isinf(DC_magnitude_val) || isnan(DC_magnitude_val)
        ...
        if abs(d_CE_val) < eps || isinf(d_CE_val) || isnan(d_CE_val)
            ...
            if abs(CE_magnitude_val) < eps || isinf(CE_magnitude_val) || isnan(CE_magnitude_val)
                ...
                F2_s_values(i) = double(subs(F2, params));
            end
        end
    end
end

%% ========== 第二部分：固定l4=0.04m，遍历k值计算 ==========
k_values = 0.01:0.01:1;  % k值范围从0.01到1，步进0.01 (避免k=0时的除零错误)
F1_k_values = zeros(size(k_values));
F2_k_values = zeros(size(k_values));
...
for i = 1:length(k_values)
    ...
    substitution_vals = [l1_val l2_val l4_fixed k_val theta1_val theta2_val];
    ...
    if abs(denominator_dGH_val) < eps || isinf(denominator_dGH_val) || isnan(denominator_dGH_val)
        ...
        if abs(d_GH_val) < eps || isinf(d_GH_val) || isnan(d_GH_val)
            ...
            F1_k_values(i) = double(subs(F1, substitution_vars, substitution_vals));
        end
    end
    ...
    if abs(DC_magnitude_val) < eps || isinf(DC_magnitude_val) || isnan(DC_magnitude_val)
        ...
        if abs(d_CE_val) < eps || isinf(d_CE_val) || isnan(d_CE_val)
            ...
            if abs(CE_magnitude_val) < eps || isinf(CE_magnitude_val) || isnan(CE_magnitude_val)
                ...
                % CE_unit_x = CE_x / CE_magnitude 和 CE_unit_y = CE_y / CE_magnitude 中检查CE_magnitude
                F2_k_values(i) = double(subs(F2, substitution_vars, substitution_vals));
            end
        end
    end
end
...
%% ========== 可视化 ==========
% 第一个图：固定k=0.5，F1和F2随s值的变化曲线
figure;
plot(s_values, F1_s_values, 'b-', 'LineWidth', 1.5);
hold on;
plot(s_values, F2_s_values, 'r--', 'LineWidth', 1.5);
xlabel('s值');
ylabel('负载力 (N)');
title('固定k=0.5时，F1和F2随s值的变化曲线');
legend('F1', 'F2');
grid on;
xlim([min(s_values), max(s_values)]);
ylim([0, max(max(F1_s_values(~isnan(F1_s_values))), max(F2_s_values(~isnan(F2_s_values)))) * 1.1]);

% 显示一些统计信息
fprintf('=== 固定k=0.5时，F1和F2随s值变化的统计信息 ===\n');
fprintf('F1最大值: %.2f N\n', max(F1_s_values(~isnan(F1_s_values))));
fprintf('F1最小值: %.2f N\n', min(F1_s_values(~isnan(F1_s_values))));
fprintf('F2最大值: %.2f N\n', max(F2_s_values(~isnan(F2_s_values))));
fprintf('F2最小值: %.2f N\n', min(F2_s_values(~isnan(F2_s_values))));

% 第二个图：固定l4=0.04m，F1和F2随k值的变化曲线
figure;
plot(k_values, F1_k_values, 'b-', 'LineWidth', 1.5);
hold on;
plot(k_values, F2_k_values, 'r--', 'LineWidth', 1.5);
xlabel('k值');
ylabel('负载力 (N)');
title('固定l4=0.04m时，F1和F2随k值的变化曲线');
legend('F1', 'F2');
grid on;
xlim([min(k_values), max(k_values)]);
ylim([0, max(max(F1_k_values(~isnan(F1_k_values))), max(F2_k_values(~isnan(F2_k_values)))) * 1.1]);

% 显示一些统计信息
fprintf('\n=== 固定l4=0.04m时，F1和F2随k值变化的统计信息 ===\n');
fprintf('F1最大值: %.2f N\n', max(F1_k_values(~isnan(F1_k_values))));
fprintf('F1最小值: %.2f N\n', min(F1_k_values(~isnan(F1_k_values))));
fprintf('F2最大值: %.2f N\n', max(F2_k_values(~isnan(F2_k_values))));
fprintf('F2最小值: %.2f N\n', min(F2_k_values(~isnan(F2_k_values))));